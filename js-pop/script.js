(() => {
  ...
  const feedbackModalPanel = feedbackModal?.querySelector(".feedback-modal-panel");
  ...
  const openFeedbackModal = (trigger) => {
    if (!feedbackModal) return;
    feedbackReturnFocus = trigger || document.activeElement;
    feedbackModal.hidden = false;
    body.classList.add("modal-open");
    window.requestAnimationFrame(() => feedbackModalPanel?.focus());
  };

  feedbackOpenButtons.forEach((button) => {
    button.addEventListener("click", () => openFeedbackModal(button));
  });
